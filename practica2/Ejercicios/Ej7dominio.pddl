(define (domain Ejercicio7)
  	...
  	(:constants 
  		...
  		manzana oscar algoritmo rosa oro - objetos
  	...
  	(:functions
  		...
  		(puntuacion-player ?p - dealer)
  		(puntuacion-total)

  		(capacidad-bolsillo ?x - personajes)
	...
	(:action PICK-UP
	    :parameters (?p - picker ?object - objetos ?z - zona)
	...
	(:action GIVE
	    :parameters (?p - dealer ?object - objetos ?z - zona ?personaje - personajes)
	    :precondition (and (player-tiene ?p ?object) (posicion-player ?p ?z) (posicion-personaje ?personaje ?z)
	    			  (< (bolsillo ?personaje) (capacidad-bolsillo ?personaje))
	    ...
	    :effect (and
	     			...
	     			(when (and (objeto-manzana ?object))
	     				(and (increase (puntuacion-player ?p) (puntuacion manzana ?personaje))
	     				(increase (puntuacion-total) (puntuacion manzana ?personaje)))
	     			)
	     			(when (and (objeto-oro ?object))
	     				(and (increase (puntuacion-player ?p) (puntuacion oro ?personaje))
	     				(increase (puntuacion-total) (puntuacion oro ?personaje)))
	     			)
	     			(when (and (objeto-rosa ?object))
	     				(and (increase (puntuacion-player ?p) (puntuacion rosa ?personaje))
	     				(increase (puntuacion-total) (puntuacion rosa ?personaje)))
	     			)
	     			(when (and (objeto-algoritmo ?object))
	     				(and (increase (puntuacion-player ?p) (puntuacion algoritmo ?personaje))
	     				(increase (puntuacion-total) (puntuacion algoritmo ?personaje)))
	     			)
	     			(when (and (objeto-oscar ?object))
	     				(and (increase (puntuacion-player ?p) (puntuacion oscar ?personaje))
	     				(increase (puntuacion-total) (puntuacion oscar ?personaje)))
	     			)

	     			(increase (bolsillo ?personaje) 1)
	     		 )
	)

	; ENTREGAR A UN ROBOT
	(:action GIVE-ROBOT
	    :parameters (?p1 - picker ?p2 - dealer ?object - objetos ?z - zona)
	    :precondition (and (player-tiene ?p1 ?object) (posicion-player ?p1 ?z) (posicion-player ?p2 ?z)
	    			  (manovacia ?p2))
	    :effect (and
	 				(not (player-tiene ?p1 ?object))
	 				(manovacia ?p1)
	 			
	 				(player-tiene ?p2 ?object)
	 				(not (manovacia ?p2))
	     		 )
	)
